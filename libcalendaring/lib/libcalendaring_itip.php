...
class libcalendaring_itip
{
            ...
            if ($attendee['role'] != 'ORGANIZER' && strcasecmp($attendee['email'], $event['attendee']) == 0) {
              ...
              if (in_array($status_lc, $this->rsvp_status)) {
                $html = html::div('rsvp-status ' . $status_lc, $this->gettext(array(
                    ...
                    'vars' => array(
                        'delegatedto' => $this->rc->Q($event['delegated-to'] ?: ($attendee['delegated-to'] ?: '?')),
                    )
                )));
              }
    ...
    public function mail_itip_inline_ui($event, $method, $mime_id, $task, $message_date = null, $preview_url = null)
    {
        ...
        if ($method == 'REPLY') {
            ...
            $update_button = html::tag('input', array(
                'type' => 'button',
                'class' => 'button',
                'onclick' => "rcube_libcalendaring.add_from_itip_mail('" . $this->rc->JQ($mime_id) . "', '$task')",
            ...
            $accept_buttons .= html::tag('input', array(
                ...
                'onclick' => "rcube_libcalendaring.decline_attendee_reply('" . $this->rc->JQ($mime_id) . "', '$task')",
        ...
        // when receiving iTip REQUEST messages:
            ...
            foreach ($this->rsvp_actions as $method) {
                $rsvp_buttons .= html::tag('input', array(
                    'type' => 'button',
                    'class' => "button $method",
                    'onclick' => "rcube_libcalendaring.add_from_itip_mail('" . $this->rc->JQ($mime_id) . "', '$task', '$method', '$dom_id')",
            ...
            if (!empty($preview_url)) {
              ...
              $rsvp_buttons .= html::tag('input', array(
                  ...
                  'onclick' => "rcube_libcalendaring.open_itip_preview('" . $this->rc->JQ($preview_url) . "', '" . $this->rc->JQ($msgref) . "')",
        ...
        // for CANCEL messages, we can:
            ...
            $button_remove = html::tag('input', array(
                'type' => 'button',
                'class' => 'button',
                'onclick' => "rcube_libcalendaring.remove_from_itip(" . rcube_output::json_serialize($event_prop) . ", '$task', '" . $this->rc->JQ($event['title']) . "')",
    ...
    function itip_object_details_table($event, $title)
    {
        ...
        $table->add('title', $this->rc->Q($event['title']));
        if ($event['start'] && $event['end']) {
            $table->add('label', $this->gettext('date'));
            $table->add('date', $this->rc->Q($this->lib->event_date_text($event)));
        }
        ...
        if ($event['location']) {
            ...
            $table->add('location', $this->rc->Q($event['location']));
        }
        ...
        if ($event['comment']) {
            ...
            $table->add('location', $this->rc->Q($event['comment']));
        }
